...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  __cs149_vec_float x, result, limit=_cs149_vset_float(9.999999f);
  __cs149_vec_int y, zeroI=_cs149_vset_int(0), count, oneI;
  __cs149_mask maskAll, maskIsNegative, maskIsNotNegative, maskIsZero, maskIsNotZero, maskLast;
  int start=N/VECTOR_WIDTH*VECTOR_WIDTH;
  int las=N-N/VECTOR_WIDTH*VECTOR_WIDTH;
  maskLast=_cs149_init_ones(las);
  for(int i=0;i<=N-VECTOR_WIDTH;i+=VECTOR_WIDTH){
    maskAll=_cs149_init_ones();
    _cs149_vset_int(oneI, 1, maskAll);
    _cs149_vload_float(x, values+i, maskAll);
    _cs149_vload_int(y, exponents+i, maskAll);
    _cs149_veq_int(maskIsZero, y, zeroI, maskAll);
    _cs149_vset_float(result, 1.f, maskIsZero);
    maskIsNotZero=_cs149_mask_not(maskIsZero);
    _cs149_vmove_float(result, x, maskIsNotZero);
    _cs149_vsub_int(y, y, oneI, maskIsNotZero);
    _cs149_vgt_int(maskIsNotNegative, y, zeroI, maskAll);
    while(1){
      int Gnum=_cs149_cntbits(maskIsNotNegative);
      if(Gnum==0) break;
      _cs149_vmult_float(result, result, x, maskIsNotNegative);
      _cs149_vsub_int(y, y, oneI, maskIsNotNegative);
      _cs149_vgt_int(maskIsNotNegative, y, zeroI, maskAll);
    }
    _cs149_vgt_float(maskIsNotNegative, result, limit, maskAll);
    _cs149_vset_float(result, 9.999999f, maskIsNotNegative);
    _cs149_vstore_float(output+i, result, maskAll);
  }
  if(N%VECTOR_WIDTH!=0){
...
float arraySumVector(float* values, int N) {
  ...
  //
  __cs149_vec_float x, result, mid;
  __cs149_mask maskAll;
  float *valuet=new float[N];
  // for(int i=0;i<N;i++)  printf("%.3f ",values[i]);
  // puts("");
  memcpy(valuet, values, sizeof(float)*N);
  int temp=VECTOR_WIDTH;
  while(temp!=1){
    for (int i=0; i<N; i+=VECTOR_WIDTH) {
      maskAll=_cs149_init_ones();
      _cs149_vload_float(x, valuet+i, maskAll);
      _cs149_hadd_float(mid, x);
      _cs149_interleave_float(result, mid);
      _cs149_vstore_float(valuet+i, result, maskAll);
    }
    // for(int i=0;i<N;i++)  printf("%.3f ",valuet[i]);
    // puts("");
    temp/=2;
  }
  float sum=0;
